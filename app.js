(function () {
  ...
  // Initializes the shared converter widget (the same one used on the
  // homepage) from the current page's conversion metadata instead of the
  // homepage defaults. Order:
  //   1. Read the page slug.
  //   2. Find that slug/metadata in the conversion registry.
  //   3. Retrieve categoryId / fromUnitId / toUnitId.
  //   4. Confirm the converter registry (categories/categoryMap) has loaded.
  //   5. Select the correct category (selectCategory - existing API).
  //   6. Populate the category's unit list (done inside selectCategory /
  //      populateSelect - existing API).
  //   7-8. Select the correct From/To unit (populateSelect with the
  //      resolved unit ids - existing API).
  //   9. Trigger the existing update/render function (updateConversion).
  // The default homepage configuration (Length/Meter/Foot) is only used
  // when no valid registry entry exists for this page.
  function applyPageConversion(pageConversion) {
    ...
    // Steps 6-8: populate the category's unit list and select From/To units
    // using the existing internal API without manual text changes or clicks.
  ...
  function resolveUnitAlias(category, value) {
    ...
    // Fallback: word-order and plural insensitive match. URL slugs are free
    // text (e.g. "mechanical-horsepower") and don't always line up token-for-
    // token with a registry unit id (e.g. "horsepower_mechanical"). Comparing
    // sorted, singularized word sets lets these resolve without hardcoding
    // any specific unit name.
  ...
  // Splits a route value or alias into normalized word tokens, naively
  // singularizes each token, then sorts them so word order doesn't matter.
  // Generic helper used only for fuzzy matching - it has no knowledge of any
  // specific unit name.
  ...
  // Very small, generic English singularizer used only to make fuzzy unit
  // matching plural-insensitive (e.g. "inches" vs "inch", "sections" vs
  // "section"). Not aware of any specific unit name.
  ...
  function rootRelativePrefix() {
    // Derive the correct "../" prefix back to the site root from this very
    // page's own <script src="...app.js"> tag, since that path is already
    // generated correctly per-page at build time (e.g. "", "../", "../../").
    // This avoids hardcoded absolute paths like "/converter" or "/" which
    // break on hosts that don't rewrite them (e.g. GitHub Pages) and would
    // also break if the site is ever served from a sub-path.
  ...
  function readSeoConversionData() {
    ...
    // Look the slug up as-is first (e.g. "watts-to-horsepower"), then fall
    // back to just the final path segment in case the slug was recorded
    // with a category-folder prefix (e.g. "power/watts-to-horsepower") that
    // doesn't match how it's keyed in the registry.
  ...
  // Generic, registry-driven fallback: works out a page's conversion directly
  // from its URL, using the same category/unit definitions (with all their
  // aliases and metric prefixes) that power the homepage converter. This
  // means every SEO page - existing or newly generated - initializes
  // correctly without needing per-page hardcoded units.
...
`;
  ...
  function initSeoConverterPage() {
    ...
    // Persist seoConversion to a global so the main initializer can apply it
    // after the category lists are rendered and select options exist.
